...
async def get_epic_game():
    ...
    try:
        async with ClientSession(headers=headers) as session:
            async with session.get(epic_url, proxy="http://127.0.0.1:60000") as res:
        ...
        for game in games:
            ...
            try:
                ...
                if not game_promotions and upcoming_promotions:
                    ...
                    start_date_utc = datetime.fromisoformat(start_date_iso)
                    start_date_local = (start_date_utc + timedelta(hours=8)).strftime("%Y-%m-%d %H:%M")  # 北京时间
                    start_date = start_date_utc.strftime(
                        "%Y-%m-%d %H:%M"
                    )

                    end_date_utc = datetime.fromisoformat(end_date_iso)  # utc时间
                    end_date_local = (end_date_utc + timedelta(hours=8)).strftime("%Y-%m-%d %H:%M")  # 北京时间
                    end_date = end_date_utc.strftime(
                        "%Y-%m-%d %H:%M"
                    )
                    msg = "\n由 {} 公司发行的游戏 {} ({}) 在 UTC 时间 {} （{}） 即将推出免费游玩，预计截至 {} (北京时间 {})。".format(
                        game_corp, game_name, game_price, start_date, start_date_local, end_date, end_date_local
                    )
                    msg_list.append(msg)
                else:
                    ...
                    msg = "[CQ:image,file={}]\n\nFREE now :: {} ({})\n{}\n此游戏由 {} 开发、{} 发行，将在 UTC 时间 {} （北京时间 {} ） 结束免费游玩，戳链接速度加入你的游戏库吧~\n{}\n".format(
                        game_thumbnail,
                        game_name,
                        game_price,
                        game_desp,
